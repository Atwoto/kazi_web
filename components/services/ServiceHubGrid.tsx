      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesData.map((service) => (
            <Card key={service.slug} className="flex flex-col h-full border-gray-100 shadow-sm hover:shadow-xl transition-shadow duration-300 rounded-2xl overflow-hidden group">
              <CardHeader className="bg-gray-50 pb-6">
                <div className="text-sm font-bold text-blue-600 uppercase tracking-wide mb-2">
                   {service.targetAudience || "Business & Pro"}
                </div>
              ...
              <CardFooter className="bg-white pt-2 pb-6">
                <Button asChild className="w-full rounded-full group-hover:bg-blue-600 transition-colors" variant="outline">
                  <Link href={`/services/${service.slug}`}>
                    View Service <ChevronRight className="w-4 h-4 ml-1" />
                  </Link>
                </Button>
              </CardFooter>
            </Card>
